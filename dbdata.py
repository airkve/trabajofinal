...
queries = {
    ...
    'get_user_shop_history':'SELECT usuarios.nombre, compras.fecha_compra, productos.producto_id, productos.nombre, compras.cantidad, compras.precio_total FROM usuarios INNER JOIN compras ON compras.usuario_id = usuarios.usuario_id INNER JOIN productos ON productos.producto_id = compras.producto_id WHERE usuarios.email = %s',
